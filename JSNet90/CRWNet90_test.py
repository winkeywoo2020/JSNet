# CRWNet JPEG90 version (test codes)
...
lr = 1e-5
batch_size = 1
logs_train_dir = '/home/amax/wyq/shuiyin/syzong'
...
def get_weightfreeze(name,shape):
    var =tf.get_variable(name,shape,initializer=tf.contrib.layers.xavier_initializer(),trainable = False)
    return var
...
conv6 = tf.nn.conv2d_transpose(conv5_concat, kernel6, output_shape = [batch_size,64,64,32],strides=[1, 2, 2, 1], padding='SAME')
...
conv8 = tf.nn.conv2d_transpose(conv7, kernel8, output_shape = [batch_size,128,128,8],strides=[1, 2, 2, 1], padding='SAME')
...
conv10 = tf.nn.conv2d_transpose(conv9, kernel10, output_shape = [batch_size,256,256,3],strides=[1, 2, 2, 1], padding='SAME')

kernelout = get_weight('kernelout',[1, 1, 3, 1])
Bout = tf.nn.conv2d(conv10, kernelout, strides=[1, 1, 1, 1], padding='SAME')
...
R_new2 = tf.minimum(R_new1,255)
G_new = -0.1687 * R1 -0.3313 * G1 + 0.5 * B1+128
G_new1 = tf.maximum(G_new,0)
G_new2 = tf.minimum(G_new1,255)
B_new = 0.5 * R1 - 0.4187 * G1 - 0.0813 * B1+128
B_new1 = tf.maximum(B_new,0)
B_new2 = tf.minimum(B_new1,255)

cb_mask = tf.nn.max_pool(G_new2 , [1,2,2,1] , [1,2,2,1] , padding='VALID')
cr_mask = tf.nn.max_pool(B_new2 , [1,2,2,1] , [1,2,2,1] , padding='VALID')
# 8*8 blocks
R_batch= tf.extract_image_patches(images=R_new2, ksizes=[1, 8, 8, 1], strides=[1, 8, 8, 1], rates=[1, 1, 1, 1],padding='VALID')
...
############################################################################################### R channel
kernel1_1 = get_weightfreeze('kernel1_1',[1, 1, 64, 64])
...
kernel5_1 = get_weightfreeze('kernel5_1',[1, 1, 64, 64])
...
RGJ = tf.concat([RRR, GGG], 3)
Oejpeg = tf.concat([RGJ, BBB], 3)
# extract watermark from B channel
RJ, GJ, BJ = tf.split(Oejpeg, 3, 3)
#decoder 128*128
kernel11 = get_weight('kernel11',[3, 3, 1, 4])
conv11 = tf.nn.conv2d(BJ, kernel11, strides=[1, 2, 2, 1], padding='SAME')
...
sess = tf.Session()
ckpt = tf.train.get_checkpoint_state("/home/amax/wyq/shuiyin/syzong")
saver.restore(sess, ckpt.model_checkpoint_path)


# sess.run(tf.global_variables_initializer())
# var = tf.global_variables()
# var_to_restore = [val for val in var if 'kernel1_1' in val.name or 'kernel5_1'in val.name]
# saver = tf.train.Saver(var_to_restore)
# ckpt = tf.train.get_checkpoint_state("/home/amax/wyq/jpegmoni/90/i90model")
# saver.restore(sess, ckpt.model_checkpoint_path)
...
#         saver_out.save(sess, checkpoint_path, global_step=step+1)
...
#     tra_lossG = sess.run([G], feed_dict={cover: train_data_x1})
    ...
    name4 = str(train_count) + "zprid" + ".txt"
    ...
    name5 = str(train_count) + "zgoal" + ".txt"
