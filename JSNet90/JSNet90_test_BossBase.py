#  JSNet 90  test codes ( BossBase dataset)
#  load the trained model and change the data path
...
train_dir1 = '/home/amax/wyq/jpegmoni/90/validation90.txt'      
train_dir2 =  '/home/amax/wyq/jpegmoni/90/validation90goal.txt'
########################################################################
...
Ih=tf.placeholder(tf.float32, [batch_size, height,width, 3])
Ig=tf.placeholder(tf.float32, [batch_size, height,width, 3])
...
# kkk = tf.reshape(conv5_3,[batch_size,1024,8,8])
...
ckpt = tf.train.get_checkpoint_state(checkpoint_dir='/home/amax/wyq/jpegmoni/70/i70model')
...
for step in range(num_train_sample):
    for j in range(batch_size):
        ...
        # train_imc1 = ndimage.imread('./19.tif')
        # train_imc2 = ndimage.imread('./19.jpg')
        # Rt, Gt, Bt = tf.split(train_imc1, 3, 3)
    ...
    ########################################################################################
    # ckpt = tf.train.get_checkpoint_state(checkpoint_dir='./9.29model60')
    # ckpt = tf.train.get_checkpoint_state(checkpoint_dir= logs_train_dir)
    # saver.restore(sess, ckpt.model_checkpoint_path)
    ########################################################################################

    tra_lossR = sess.run([RRR], feed_dict={Ih:train_data_x1, Ig:train_data_x2})
    tra_lossG = sess.run([GGG], feed_dict={Ih: train_data_x1, Ig: train_data_x2})
    tra_lossB = sess.run([BBB], feed_dict={Ih: train_data_x1, Ig: train_data_x2})
    # tra_loss = sess.run([Oe], feed_dict={Ih: train_data_x1, Ig: train_data_x2})
    ...
    # tra_loss = np.reshape(tra_loss, (256,256))
    ...
    # np.savetxt("19r.txt", tra_lossR)
    # np.savetxt("19g.txt", tra_lossG)
    # np.savetxt("19b.txt", tra_lossB)
    # np.savetxt("19.txt", tra_loss)
    # _, tra_loss = sess.run([train_op, loss], feed_dict={Ih: train_data_x1, Ig: train_data_x2})
    tra_loss = sess.run(loss, feed_dict={Ih: train_data_x1, Ig: train_data_x2})
    if (step + 1) % 1 == 0:
        print('Step %d, validation loss = %.4f' % (step, tra_loss))
        # print('validation loss = %.2f' % (tra_loss))

    #
    # _, tra_loss = sess.run([train_op, loss], feed_dict={Ih:train_data_x1, Ig:train_data_x2})
    #
    # if (step + 1) % 10 == 0:
    #     print('Step %d, train loss = %.2f' %(step+1, tra_loss))
    #
    # if (step + 1) % 5000 == 0:
